'use client'

import { Button } from "@/components/ui/button"
import { Label } from "@/components/ui/label"
import { Input } from "@/components/ui/input"
import Link from "next/link"
import { Separator } from "@/components/ui/separator"

export default function Component() {
  return (
    <div className="flex flex-col h-screen justify-center items-center bg-gray-100 dark:bg-gray-900">
      <div className="w-full max-w-md px-8 py-6 bg-white dark:bg-gray-800 rounded shadow-md">
        <h2 className="text-2xl font-semibold text-center text-gray-700 dark:text-white">Login to your account</h2>
        <div className="mt-4">
          <Link href='/dashboard'>
            <Button className="w-full flex items-center justify-center">
              <GithubIcon className="h-5 w-5 mr-2" />
              Login with GitHub
            </Button>
          </Link>
        </div>
        <div className="mt-10">
          <div className="relative">
            <Label className="text-sm text-gray-700 dark:text-gray-200" htmlFor="email">
              Email address
            </Label>
            <Input className="mt-1 w-full" id="email" placeholder="john@example.com" required type="email" />
          </div>
          <Button variant='secondary' className="w-full mt-4">Send Magic Link</Button>
        </div>
      </div>
      <footer className="mt-6 text-center text-gray-400 dark:text-gray-200">
        <Link className="underline hover:text-gray-600 dark:hover:text-gray-300" href="#">
          Terms of Service
        </Link>
        <span className="mx-2">•</span>
        <Link className="underline hover:text-gray-600 dark:hover:text-gray-300" href="#">
          Privacy Policy
        </Link>
      </footer>
    </div>
  )
}

function GithubIcon(props: any) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
      <path d="M9 18c-4.51 2-5-2-7-2" />
    </svg>
  )
}
